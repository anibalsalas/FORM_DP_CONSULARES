...
// src/app/services/ocmre.service.ts
var FichaService = class _FichaService {
  http;
  apiUrl = "/api/ocmre";
  ...
  //     return this.http.get<FichaEntity[]>(`${this.apiUrl}/listarFichas`);
  //   }
  listarFichas() {
    console.log("\u{1F50D} Intentando listar fichas...");
    console.log("\u{1F511} Token actual:", localStorage.getItem("token"));
    return this.http.get(`${this.apiUrl}/listarFichas`).pipe(tap(() => console.log("\u2705 Listado de fichas exitoso")), catchError((error) => {
  ...
  registrarFicha(ficha) {
    return this.http.post(`${this.apiUrl}/guardarFicha`, ficha);
  }
  eliminarFicha(idFicha) {
    return this.http.delete(`${this.apiUrl}/eliminarFicha/${idFicha}`);
  }
  // Obtener datos del padrón
  obtenerPadron() {
    return this.http.get(`${this.apiUrl}/padron`).pipe(tap(() => console.log("\u{1F4E5} Padr\xF3n cargado")), catchError((error) => {
      console.error("\u274C Error al cargar el padr\xF3n:", error);
      return throwError(() => error);
    }));
  }
  // obtenerFichaPorId(id: number): Observable<FichaEntity> {
  //   return this.http.get<FichaEntity>(`${this.apiUrl}/obtenerFicha/${id}`);
  // }
  verificarEntidadExistente(nomEntidad) {
    return this.http.get(`${this.apiUrl}/existeEntidad`, {
      params: { nomEntidad }
    });
  }
  obtenerFichaCompletaPorId(id) {
    return this.http.get(`/api/ficha/obtenerFichaCompleta/${id}`);
  }
  static \u0275fac = function FichaService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _FichaService)(\u0275\u0275inject(HttpClient));
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _FichaService, factory: _FichaService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(FichaService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
//# sourceMappingURL=chunk-NLOR5DDK.js.map
